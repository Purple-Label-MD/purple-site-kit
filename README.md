...
| **Checkout entry mode** | `checkoutMode` on your brand config (`lib/brand/`) | Set `checkoutMode: "pay-first"` when your pack says pay-first (`clinical_after_pay`): the intake host then requests only the **qualification** layer (`/instrument/resolve?phase=qualification`), completes after your screening questions, and hands off to the hosted checkout with the `journey_id`; the clinical intake runs after payment (`phase=clinical`, resumed via `X-Journey-Id`). Leave it unset (`questionnaire-first`) to run the whole instrument before checkout. GROWTH brands additionally pick `funnelMode: "quiz-first"` or `"buy-first"` for the storefront entry. |
